...
const backend_url = "https://api-custom-chatbot.vercel.app"

fetch(`${backend_url}/v1/settings/chatbots/${window.config.chatbot_id}/fetch`, {
  method: 'GET',
  headers: {
  ...
  .then(data => {
    ...
    if(data.chatbot_status == false){
        return;
    }
    initialMessage = data.initial_message;
    chatbot_name = data.chatbot_name;
    chatbot_dp = data.chatbot_dp_url;
    chatbot_toggle_picture = data.chatbot_button_img_url;
    user_chatmessage_color = data.user_chatmessage_color;
    bot_chatmessage_color = data.bot_chatmessage_color;
    ...
    function createElement(elementType, attributes = {}, content = "") {
        ...
        // If the content is an image source, create an <img> element
    ...
    const image = document.createElement('img');
    image.src = 'https://static.wixstatic.com/media/29eacb_714929a211c640b6a455177d5daaf22b~mv2.png'; // Replace 'path_to_your_image.jpg' with the actual path to your image
    image.style.width = '20px'; // Set the width as desired (e.g., 50px)
    ...
    if (userMessage.trim() !== '') {
        ...
        const apiUrl = `${backend_url}/v1/chat`;
        const queryParams = { chatbot_id:window.config.chatbot_id,query: userMessage};
